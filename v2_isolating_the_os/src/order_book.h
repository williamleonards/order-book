...
class OrderBook {
   public:
	...
	// Warmup our data structures with n elements each
	void warmup(std::size_t n) {
		buy_orders_.reserve(n);
		sell_orders_.reserve(n);

		// uint64_t hashes to itself in most implementations
		for (std::size_t i = 0; i < n; ++i) {
			buy_orders_.emplace(i, buy_heap_.end());
			sell_orders_.emplace(i, sell_heap_.end());
		}

		for (std::size_t i = 0; i < n; ++i) {
			buy_heap_.emplace(
				BookEntry{.id = 0, .type = Side::BUY, .amt = 0, .price = i});
			sell_heap_.emplace(
				BookEntry{.id = 0, .type = Side::SELL, .amt = 0, .price = i});
		}
